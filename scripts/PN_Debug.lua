...
PN = PN or {}   -- needed because we register PN:cmdPnShowEffects below
...
local _DBG_STAGE_ORDER = {
	  ...
	  SHEEP   = { LAMB=1, GROWER=2, EWE_LACT=3, EWE_DRY=4, RAM=5, WETHER_FINISH=6, RAM_ADULT=7, WETHER_OVERAGE=8, EWE_OVERAGE=9, OVERAGE=10, DEFAULT=99 },
	  ...
	  GOAT    = { KID=1, DOE_LACT=2, DOE_DRY=3, WETHER_FINISH=4, BUCK=5, BUCK_ADULT=6, WETHER_OVERAGE=7, DOE_OVERAGE=8, DEFAULT=99 },
	  CHICKEN = { CHICK=1, LAYER=2, ROOSTER=3, DEFAULT=99 },
	  HORSE   = { FOAL=1, WEANLING=2, MARE_LACT=3, MARE_DRY=4, YEARLING=5, COLT=6, STALLION=7, GELDING=8, GELDING_OVERAGE=9, MARE_OVERAGE=10, OVERAGE=11, DEFAULT=99 },
	  ANIMAL  = { DEFAULT=1 }
}
...
-- Normalize ambiguous or legacy stage labels to canonical overlay/debug ones
local function _normalizeStage(species, gender, stage, flags, ageM)
    local sp = tostring(species or "ANIMAL"):upper()
    local st = tostring(stage or ""):upper()
    local sx = (gender == "male" or gender == "female") and gender or nil
    local isCast = flags and (flags.castrated == true or flags.isCastrated == true or flags.isCast == true)

    -- If Core (or our resolver) returned legacy OVERAGE, split by sex
    if sp == "COW" and st == "OVERAGE" then
        if sx == "male" or isCast then return "STEER_OVERAGE" else return "COW_OVERAGE" end
    end

    -- already canonical? leave as-is
    if st == "STEER_OVERAGE" or st == "COW_OVERAGE" then
        return st
    end
...
-- ---------- Pricing helpers (debug; align with overlay intent) ----------
local function _dbgSpeciesMeta(species)
    local SU = tostring(species or "ANIMAL"):upper()
    -- Prefer PN_Core.meta, fall back to PN_Settings.meta (or PN_Settings.load())
    local meta = (PN_Core and PN_Core.meta and PN_Core.meta[SU])
              or (PN_Settings and PN_Settings.meta and PN_Settings.meta[SU])
    if (not meta) and PN_Settings and PN_Settings.load then
        local ok, cfg = pcall(PN_Settings.load)
        if ok and type(cfg)=="table" and cfg.meta then
            meta = cfg.meta[SU]
        end
    end
    return meta
...
-- return suffix string like " | price -15%%" (or ""), and a detail reason
local function _dbgPriceSuffixForStage(entry, species, stage)
    local meta = _dbgSpeciesMeta(species)
    if not (meta and meta.sale) then return "", "no meta.sale" end
    local floor = tonumber(meta.sale.overagePenaltyFloor or 1) or 1
    if floor >= 0.999 then return "", "floor>=1" end

    local SU  = tostring(species or ""):upper()
    local STU = tostring(stage or ""):upper()
    if STU == "" then return "", "no stage" end

    -- If explicit OVERAGE stage, always apply the floor
    if STU:find("OVERAGE", 1, true) then
        local pct = math.floor((1 - math.min(1, floor)) * 100 + 0.5)
        if pct >= 1 then return string.format(" | price -%d%%", pct), "explicit OVERAGE" else return "", "pct<1" end
    end

    -- Otherwise, inspect snapshot animals for fraction over thresholds
    local overM = tonumber(meta.overageM   or 1e9) or 1e9 -- castrated male threshold (months)
    local snrM  = tonumber(meta.overageSnr or 1e9) or 1e9 -- senior female threshold (months)

    local animals = entry and entry.__pn_last and entry.__pn_last.animals
    if type(animals) ~= "table" or #animals == 0 then
        return "", "no snapshot animals"
    end
    ...
    if fAll == 0 and mAll == 0 then return "", "no female or castrated males in this stage" end
    ...
    if factor >= 0.995 then return "", "factor≈1" end
    local pct = math.floor((1 - factor) * 100 + 0.5)
    return string.format(" | price -%d%%", pct), string.format("fracF=%.2f fracM=%.2f floor=%.2f", fracF, fracM, floor)
end
...
function PN_Debug:cmdStages(idxStr)
    ...
    for _, c in pairs(clusters) do
        if type(c)=="table" and not c.isDead then
            ...
            stage = _normalizeStage(species, gender, stage, flags, ageM)local avgW       = _cAvgWeightKg(c)
    ...
    for _, st in ipairs(keys) do
        ...
        local avgM = (t.head > 0) and (t.ageSum / t.head) or 0
                local suf, why = _dbgPriceSuffixForStage(e, species, st)
Logging.info("[PN]   stage=%-8s | head=%3d | avgW=%7.2f kg | age=avg %.1f m (min %.1f, max %.1f)%s",
            tostring(st), t.head, avgW, avgM, t.ageMin, t.ageMax, (suf ~= "" and (" "..suf) or ""))
    end
    Logging.info("[PN] -----------------------------------------------")
end
...
function PN_Debug:cmdStagesWhy(idxStr)
    ...
    for _, c in pairs(clusters) do
        if type(c)=="table" and not c.isDead then
            ...
            stage = _normalizeStage(species, gender, stage, flags, ageM)local reason  = hasFlag and "[flag]" or (byType and "[subType]") or ""
...
function PN_Debug:cmdPNReload()
    ...
    local cfg = PN_Settings.load(true)  -- was PN_Settings.load()
    PN_Core.init(cfg)
...
-- Price audit for an entry: pnPriceAudit <index>
function PN_Debug:cmdPriceAudit(idxStr)
    local idx = tonumber(idxStr or "")
    if idx == nil then
        Logging.info("[PN] Usage: pnPriceAudit <index>  (see pnDumpHusbandries)")
        return
    end
    local list = (PN_HusbandryScan and PN_HusbandryScan.getAll and PN_HusbandryScan.getAll()) or {}
    local e = list[idx]
    if not e or not e.clusterSystem then
        Logging.info("[PN] pnPriceAudit: no entry/cluster at index %s", tostring(idx))
        return
    end
    local species = _dbgNormSpecies(e.type or (e.__pn_last and e.__pn_last.species) or "ANIMAL")
    local meta = _dbgSpeciesMeta(species) or {}
    local floor = (meta.sale and tonumber(meta.sale.overagePenaltyFloor or 1)) or 1
    local overM = tonumber(meta.overageM   or 1e9) or 1e9
    local snrM  = tonumber(meta.overageSnr or 1e9) or 1e9
    Logging.info("[PN] PriceAudit '%s' [%s] | floor=%.2f overageM=%s snrF=%s", tostring(e.name or "?"), species, floor, tostring(overM), tostring(snrM))
    ...
    if e.__pn_last and type(e.__pn_last.animals) == "table" and #e.__pn_last.animals > 0 then
        for _, a in ipairs(e.__pn_last.animals) do
            local st = tostring(a.stage or ""):upper()
            if st ~= "" then
                local head = tonumber(a.head or a.count or a.n or 0) or 0
                if head > 0 then
                    local gRaw = tostring(a.gender or a.sex or ""):lower()
                    ...
                    local t = agg[st]; if not t then t = { head=0, fAll=0, fOver=0, mAll=0, mOver=0 } ; agg[st]=t end
                    t.head = t.head + head
                    if g == "female" then
                        t.fAll = t.fAll + head
                        if age >= snrM then t.fOver = t.fOver + head end
                    elseif g == "male" and isCast then
                        t.mAll = t.mAll + head
                        if age >= overM then t.mOver = t.mOver + head end
                    end
                end
            end
        end
    ...
    if not usedAnimals then
        ...
        if okC and type(clusters)=="table" then
            for _, c in pairs(clusters) do
                if type(c)=="table" and not c.isDead then
                    local head = tonumber(c.numAnimals or c.n or c.count or c.head or 1) or 1
                    local ageM = _cAgeMonths(c) or 0
                    local gender = _cGender(c)
                    local isCast = _cCastrated(c) or _cLooksLikeSteer(c)
                    local flags  = { lactating=_cLactating(c), castrated=isCast }
                    local st     = _dbgResolveStage(species, gender, ageM, flags)
                    st = tostring(st):upper()
                    local t = agg[st]; if not t then t = { head=0, fAll=0, fOver=0, mAll=0, mOver=0 } ; agg[st]=t end
                    t.head = t.head + head
                    if gender == "female" then
                        t.fAll = t.fAll + head
                        if ageM >= snrM then t.fOver = t.fOver + head end
                    elseif gender == "male" and isCast then
                        t.mAll = t.mAll + head
                        if ageM >= overM then t.mOver = t.mOver + head end
                    end
                end
            end
        end
    ...
    for _, st in ipairs(keys) do
        local t = agg[st]
        local suf, why = _dbgPriceSuffixForStage(e, species, st)
        local fracF = (t.fAll > 0) and (t.fOver/t.fAll) or 0
        local fracM = (t.mAll > 0) and (t.mOver/t.mAll) or 0
        Logging.info("[PN]   %-12s | head=%3d | females over=%d/%d (%.0f%%) | castM over=%d/%d (%.0f%%) %s %s",
            tostring(st), t.head or 0,
            t.fOver or 0, t.fAll or 0, (fracF*100),
            t.mOver or 0, t.mAll or 0, (fracM*100),
            (suf ~= "" and ("|"..suf) or ""), (why and ("["..tostring(why).."]") or ""))
    end
end

function PN_Core:cmdDumpTotals(id)
  local idx = tonumber(id or 1) or 1
  local barns = PN_HusbandryScan and PN_HusbandryScan.getAll() or {}
  local e = barns[idx]
  if not e then return Logging.info("[PN] No barn at index %d", idx) end
  local t = (e.__pn_last and e.__pn_last.totals) or {}
  Logging.info("[PN] Totals for #%d '%s': head=%s nut=%.3f adg=%.3f DMI/hd=%.2f  DMIt=%.1f  E=%.1f MJ  P=%.3f kg  E/hd=%.1f MJ  P/hd=%.3f kg",
    idx, tostring(e.name or "?"),
    tostring(t.animals or "?"), tonumber(t.nut or 0), tonumber(t.adg or 0),
    tonumber(t.intakeKgHd or 0), tonumber(t.DMIt or 0),
    tonumber(t.energyMJ or 0), tonumber(t.proteinKg or 0),
    (t.perHead and tonumber(t.perHead.energyMJ or 0) or 0),
    (t.perHead and tonumber(t.perHead.proteinKg or 0) or 0)
  )
end

-- Show merged runtime configuration for PN_GrazingFeeder
function PN_Debug:cmdGrazeCfg()
    local runtime = nil
    if PN_GrazingFeeder and PN_GrazingFeeder.getCfg then
        local ok, cfg = pcall(PN_GrazingFeeder.getCfg, PN_GrazingFeeder)
        if ok and type(cfg)=="table" then runtime = cfg end
    end
    local settings = nil
    if PN_Settings and PN_Settings.load then
        local ok, cfg = pcall(PN_Settings.load)
        if ok and type(cfg)=="table" and type(cfg.grazingFeeder)=="table" then settings = cfg.grazingFeeder end
    end

    Logging.info("[PN] ---- Grazing Feeder Config ----")
    if settings then
        Logging.info("[PN] Settings (PN_Settings.grazingFeeder):")
        for k,v in pairs(settings) do Logging.info("[PN]   %-16s = %s", tostring(k), tostring(v)) end
    else
        Logging.info("[PN] Settings: (none found in PN_Settings.grazingFeeder)")
    end

    if runtime then
        Logging.info("[PN] Runtime (merged, active):")
        for k,v in pairs(runtime) do Logging.info("[PN]   %-16s = %s", tostring(k), tostring(v)) end
    else
        Logging.info("[PN] Runtime: PN_GrazingFeeder.getCfg() not available")
    end
    Logging.info("[PN] --------------------------------")
end

-- Control grazing feeder via PN_Debug: pnGraze on|off|debug <0|1>|status
function PN_Debug:cmdGraze(arg1, arg2)
    if not PN_GrazingFeeder then
        Logging.info("[PN] pnGraze: PN_GrazingFeeder not available")
        return
    end
    if PN_GrazingFeeder.cmdGraze then
        -- delegate to feeder's internal handler
        pcall(PN_GrazingFeeder.cmdGraze, PN_GrazingFeeder, arg1, arg2)
    else
        -- minimal fallback
        local sub = tostring(arg1 or "status"):lower()
        if sub == "on" then
            PN_GrazingFeederEnabled = true
            Logging.info("[PN] Grazing feeder: enabled (fallback)")
        elseif sub == "off" then
            PN_GrazingFeederEnabled = false
            Logging.info("[PN] Grazing feeder: disabled (fallback)")
        else
            Logging.info("[PN] pnGraze: try 'on|off|debug <0|1>|status'")
        end
    end
end
...
-- Register once (somewhere in your existing registration block)
-- addConsoleCommand("pnShowEffects", "Show PN nutrition effects summary", "cmdPnShowEffects", PN)

local function _pnSafeCount(t)
...
local function _pnIterHusbandries()
    -- Prefer the game’s list if present
    if g_currentMission and g_currentMission.husbandries then
        return pairs(g_currentMission.husbandries)
    end
    -- Fallbacks (if you cache entries in PN_Core)
    if PN_Core and PN_Core._husList then
        return ipairs(PN_Core._husList)
    end
    if PN_Core and PN_Core._entries then
        return pairs(PN_Core._entries)
    end
    -- Nothing to iterate
    return function() return nil end
end

local function _milkString(e, totals, milkMul)
    local show=false
    if e.isMilking or e.milkEnabled then show=true
    else
        local st = (totals and tostring(totals.stage):upper()) or ""
        if st:find("LACT",1,true) then show=true end
    end
    return show and string.format(" | Milk x%.2f", milkMul or 1.0) or ""
end

-- NEW: per-stage printer using entry.__pn_last.animals snapshot (ASCII only)
local function _printStages(e)
    local animals = (e.__pn_last and e.__pn_last.animals) or {}
    if type(animals) ~= "table" or next(animals) == nil then
        return 0
    end

    -- aggregate by stage (+gender if you ever want to split later)
    local byStage = {}
    for _, a in ipairs(animals) do
        local st  = tostring(a.stage or "UNKNOWN")
        local g   = tostring(a.gender or "")
        local key = st .. "|" .. g

        local head = tonumber(a.head or 1) or 1
        local avgW = tonumber(a.avgW or 0) or 0

        local dst = byStage[key]
        if not dst then
            dst = { stage = st, gender = g, n = 0, wSum = 0 }
            byStage[key] = dst
        end
        dst.n    = dst.n + head
        dst.wSum = dst.wSum + (avgW * head)
    end

    local last    = e.__pn_last or {}
    local barnNut = tonumber(last.nutRatio or e.__pn_nutRatio) or 0
    local totals  = e.__pn_totals
    local species = (totals and totals.species) or "COW"

    local printed = 0
    for _, dst in pairs(byStage) do
        local n = dst.n or 0
        if n > 0 then
            local avgW = (dst.wSum > 0 and (dst.wSum / n)) or 0

            -- stage ADG: prefer PN_Core.adgFor(species, stage, avgW, nut)
            local stageAdg = 0
            if PN_Core and PN_Core.adgFor then
                local okA, v = pcall(PN_Core.adgFor, PN_Core, species, dst.stage, avgW, barnNut)
                if okA and type(v) == "number" then stageAdg = v end
            end

            -- show the barn’s current multiplier (what growth is using this frame)
            local mul = tonumber(last.adgMul or e.__pn_adgMul) or 0

            print(string.format("    - [%s] | head=%d | avgW=%.2fkg | Nut~%d%% | ADG x%.2f | ADG=%.3f kg/d",
                dst.stage, n, avgW,
                (function(x) x=tonumber(x or 0) or 0 if x<0 then x=0 elseif x>1 then x=1 end return math.floor(x*100+0.5) end)(barnNut),
                (mul>0 and mul or 0),
                stageAdg
            ))
            printed = printed + 1
...
-- Small helpers for showEffects
local function _pairsOrEmpty(t) if type(t)=="table" then return pairs(t) end return function() return nil end end
local function _pct01(x) x=tonumber(x or 0) or 0 if x<0 then x=0 elseif x>1 then x=1 end return math.floor(x*100+0.5) end

local function _keyForEntry(e)
    if e.placeableId then return "pid:"..tostring(e.placeableId) end
    if e.id then return "id:"..tostring(e.id) end
    if e.placeable then return "plc:"..tostring(e.placeable) end
    local n = tostring(e.name or "Husbandry")
    local sp = (e.__pn_totals and e.__pn_totals.species) or ""
    return "nm:"..n.."|"..tostring(sp)
end

local function _chooseBetter(a,b)
    if a==nil then return b end
    if b==nil then return a end
    local ah=(a.__pn_totals and tonumber(a.__pn_totals.animals)) or 0
    local bh=(b.__pn_totals and tonumber(b.__pn_totals.animals)) or 0
    if ah~=bh then return (ah>bh) and a or b end
    local af=(a.__pn_last and a.__pn_last.nutRatio) and 1 or 0
    local bf=(b.__pn_last and b.__pn_last.nutRatio) and 1 or 0
    if af~=bf then return (af>bf) and a or b end
    return a
end

local function _collectPool()
    local pool = {}
    if PN_Core and PN_Core._entries then
        for _, holder in _pairsOrEmpty(PN_Core._entries) do
            local e = holder and holder.ref or nil
            if type(e)=="table" then
                local k=_keyForEntry(e); pool[k]=_chooseBetter(pool[k], e)
            end
        end
    end
    if next(pool)==nil and g_currentMission and g_currentMission.husbandries then
        for _, hus in _pairsOrEmpty(g_currentMission.husbandries) do
            local e = hus and (hus.pnEntry or hus) or nil
            if type(e)=="table" then
                local k=_keyForEntry(e); pool[k]=_chooseBetter(pool[k], e)
            end
        end
...
-- The command implementation (ASCII only; uses '-' and '~')
function PN:cmdPnShowEffects(arg)
    local wantStages = (type(arg)=="string" and arg:lower():find("stages",1,true)~=nil)
    print("[PN] Effects summary:")

    local pool = _collectPool()
    local printed = 0

    for _, e in _pairsOrEmpty(pool) do
        local last   = e.__pn_last or {}
        local totals = e.__pn_totals
        local head   = (totals and tonumber(totals.animals)) or 0
        if head>0 then
            local name  = tostring(e.name or e.id or "Husbandry")
            local stage = (totals and tostring(totals.stage)) or ""
            local nut   = tonumber(last.nutRatio or e.__pn_nutRatio) or 0
            local adg   = tonumber(last.effADG) or tonumber(e.__pn_adg) or 0
            local mul   = tonumber(last.adgMul or e.__pn_adgMul) or 0
            local cond  = tonumber(e.__pn_condition) or 1.0
            local milk  = tonumber(e.__pn_milkMul) or 1.0

            local milkStr = ""
            do
                local showMilk=false
                if e.isMilking or e.milkEnabled then showMilk=true
                else
                    local st = (totals and tostring(totals.stage):upper()) or ""
                    if st:find("LACT",1,true) then showMilk=true end
                end
                if showMilk then milkStr = string.format(" | Milk x%.2f", milk) end
            end

            print(string.format("  - %s%s | head=%d | Nut~%d%% | ADG x%.2f | Cond=%.2f%s | ADG=%.3f kg/d",
                name,
                (stage~="" and (" ["..stage.."]") or ""),
                head, _pct01(nut), (mul>0 and mul or 0), cond, milkStr, adg))
            printed = printed + 1

            if wantStages then
                local count = _printStages(e)
                if count == 0 then
                    print("    (no per-stage snapshot cached yet)")
                end
            end
        end
    end
    ...
    if printed == 0 then
        print("[PN] No non-empty husbandries found (tip: ensure barns have updated this session).")
    end
end
...
if addConsoleCommand ~= nil then
    ...
    addConsoleCommand("pnDumpTotals", 		  "Dump PN barn totals", 						  "cmdDumpTotals", 		   PN_Debug)
    ...
    addConsoleCommand("pnPriceAudit",        "Audit price suffix per stage for an entry", "cmdPriceAudit",        PN_Debug)
	addConsoleCommand("pnStagesWhy", 		  "Explain steer detection & stage per cluster", 	"cmdStagesWhy", 		PN_Debug)
    addConsoleCommand("pnGrazeCfg",        "Show grazing feeder merged config",        "cmdGrazeCfg",         PN_Debug)
    addConsoleCommand("pnGraze",           "Grazing feeder controls",                  "cmdGraze",            PN_Debug)
	addConsoleCommand("pnShowEffects", 	"Show PN nutrition effects summary", "cmdPnShowEffects", PN)
