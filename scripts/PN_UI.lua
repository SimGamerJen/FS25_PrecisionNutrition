-- PN_UI.lua — overlay (inert if RL missing)
if rawget(_G, "CompatRL") and not CompatRL.hasRL then
  PN_UI = PN_UI or {}
  function PN_UI:cmdOverlay(...) print("[PN] Disabled: requires Realistic Livestock.") end
  if addConsoleCommand then addConsoleCommand("pnOverlay","Overlay controls (disabled)","cmdOverlay", PN_UI) end
  return PN_UI
end

-- >>> BEGIN ORIGINAL PN_UI CONTENT (with Effects row & minor fixes)
...
PN_UI.cfg = PN_UI.cfg or {
  ...
  preferLiveRows = true,
  -- NEW: Effects line controls
  showEffectsRow = true,        -- show the Effects summary line
  effectsOnlySingle = true      -- only show in single-barn mode to avoid clutter
}
...
-- Unit helpers
local function _isFinite(n) return type(n)=="number" and n==n and n~=math.huge and n~=-math.huge end
local function _toMJ(x, unit)  -- accepts MJ (default), Mcal, kcal
  if not _isFinite(x) then return nil end
  unit = (unit or "MJ"):lower()
  if unit == "mj" then return x end
  if unit == "mcal" then return x * 4.184 end
  if unit == "kcal" then return x * 0.004184 end
  return x
end
local function _toKg(x, unit)  -- accepts kg (default), g
  if not _isFinite(x) then return nil end
  unit = (unit or "kg"):lower()
  if unit == "kg" then return x end
  if unit == "g"  then return x / 1000.0 end
  return x
end

-- Nested-aware numeric picker (kept from earlier)
local function _numPick(tbl, keys)
  ...
  for _, k in ipairs(keys) do
    local raw = getPath(tbl, k)
    if raw == nil then raw = tbl[k] end
    local v = tonumber(raw)
    if _isFinite(v) then return v end
  end
  return nil
end

-- Robust energy picker → MJ (tries totals first, then perHead)
local function _pickEnergyMJ(t)
  if type(t) ~= "table" then return nil end
  -- Direct MJ fields (totals)
  local E = _numPick(t, {"E","energyMJ","E_MJ","energy","ME_MJ","NEL_MJ","NE_MJ","DE_MJ","GE_MJ","totals.energy.MJ"})
  if _isFinite(E) then return _toMJ(E, "MJ") end
  -- Mcal totals
  local Emcal = _numPick(t, {"E_Mcal","ME_Mcal","NE_Mcal","DE_Mcal","totals.energy.Mcal"})
  if _isFinite(Emcal) then return _toMJ(Emcal, "Mcal") end
  -- kcal totals
  local Ekcal = _numPick(t, {"E_kcal","totals.energy.kcal"})
  if _isFinite(Ekcal) then return _toMJ(Ekcal, "kcal") end
  -- Per-head (MJ)
  local Eh = _numPick(t, {"perHead.energyMJ","perHead.E_MJ","perHead.ME_MJ","perHead.NE_MJ","perHead.DE_MJ"})
  if _isFinite(Eh) then return _toMJ(Eh, "MJ") end
  -- Per-head (Mcal/kcal)
  local Ehmcal = _numPick(t, {"perHead.E_Mcal","perHead.ME_Mcal","perHead.NE_Mcal","perHead.DE_Mcal"})
  if _isFinite(Ehmcal) then return _toMJ(Ehmcal, "Mcal") end
  local Ehkcal = _numPick(t, {"perHead.E_kcal"})
  if _isFinite(Ehkcal) then return _toMJ(Ehkcal, "kcal") end
  return nil
end

-- Robust protein picker → kg (tries totals first, then perHead; supports grams)
local function _pickProteinKg(t)
  if type(t) ~= "table" then return nil end
  -- Totals in kg
  local P = _numPick(t, {"P","proteinKg","P_kg","protein","totals.protein.kg"})
  if _isFinite(P) then return _toKg(P, "kg") end
  -- Totals in grams
  local Pg = _numPick(t, {"protein_g","totals.protein.g"})
  if _isFinite(Pg) then return _toKg(Pg, "g") end
  -- Per-head kg
  local Ph = _numPick(t, {"perHead.proteinKg","perHead.P_kg"})
  if _isFinite(Ph) then return _toKg(Ph, "kg") end
  -- Per-head grams
  local Phg = _numPick(t, {"perHead.protein_g"})
  if _isFinite(Phg) then return _toKg(Phg, "g") end
  -- Crude protein aliases
  local CP = _numPick(t, {"CP_kg","totals.CP.kg","perHead.CP_kg"}) or nil
  if _isFinite(CP) then return _toKg(CP, "kg") end
  local CPg = _numPick(t, {"CP_g","totals.CP.g","perHead.CP_g"}) or nil
  if _isFinite(CPg) then return _toKg(CPg, "g") end
  return nil
end

-- Prefer freshest PN totals wherever they live; fall back to PN_Core.getTotals
function PN_UI._getBarnTotals(e)
  if not e then return {} end

  -- Most recent snapshot from PN_Core.updateHusbandry
  if e.__pn_last and type(e.__pn_last.totals) == "table" then
    return e.__pn_last.totals
  end

  -- Barn-level caches we’ve seen in different PN builds
  if type(e.__pn_totals) == "table" then return e.__pn_totals end
  if e.placeable and type(e.placeable.__pn_totals) == "table" then
    return e.placeable.__pn_totals
  end

  -- Ask PN_Core for anything it knows about this barn/placeable/cluster
  local subj = e.placeable or e.clusterSystem or e
  if PN_Core and PN_Core.getTotals and subj then
    local ok, t = pcall(PN_Core.getTotals, subj)
...
local function _fmt(n, d)
  return string.format("%."..tostring(d or 0).."f", tonumber(n or 0) or 0)
end

-- ---- overlay toggle & key polling (Alt+N open/close; Alt+B prev, Alt+M next) ----

function PN_UI.toggle()
  local was = PN_UI.enabled
  PN_UI.enabled = not was
  if PN_UI.enabled then
    -- Opening overlay: build the list and start on first barn in SINGLE mode
    PN_UI.mode = "single"
    local list = PN_UI.buildBarnList()
    if #list > 0 then
      PN_UI._selectBarn(1)
    else
      PN_UI._barnIdx = 0
...
-- FIX: remove stray 'local' newline; define properly
local function pollOverlayKeys(dt)
  ...
  -- ALT+N: strict toggle (open/close). On open, jump to first barn in SINGLE mode.
  ...
  -- While overlay is ON: ALT+B -> previous barn, ALT+M -> next barn
  if not PN_UI.enabled then return end
...
-- --- price suffix helper (robust; explicit OVERAGE always shows; blended with snapshot if present) --
local function _pn_priceSuffixForStage(entry, species, stage)
  if not species or not stage then return "" end
  local SU  = tostring(species or ""):upper()
  local STU = tostring(stage or ""):upper()

  -- Resolve meta
  local meta = (PN_Core and PN_Core.meta and PN_Core.meta[SU])
            or (PN_Settings and PN_Settings.meta and PN_Settings.meta[SU])
            or (function()
                 if PN_Settings and PN_Settings.load then
                   local ok, cfg = pcall(PN_Settings.load)
                   if ok and cfg and cfg.meta then return cfg.meta[SU] end
                 end
                 return nil
               end)()
  if not (meta and meta.sale) then return "" end

  local floor = tonumber(meta.sale.overagePenaltyFloor or 1) or 1
  if floor >= 0.999 then return "" end

  -- Explicit overage stage => show floor directly
  if STU:find("OVERAGE", 1, true) then
    local pct = math.floor((1 - math.min(1, floor)) * 100 + 0.5)
    if pct >= 1 then return string.format(" | price -%d%%", pct) end
    return ""
  end

  -- If we have a snapshot, compute blended factor for this stage
  local animals = entry and entry.__pn_last and entry.__pn_last.animals
  if type(animals) ~= "table" or #animals == 0 then
    return "" -- nothing to blend
  end

  local overM = tonumber(meta.overageM   or 1e9) or 1e9   -- castrated males threshold (months)
  local snrM  = tonumber(meta.overageSnr or 1e9) or 1e9   -- senior females threshold (months)

  local fAll,fOver,mAll,mOver = 0,0,0,0

  for _, a in ipairs(animals) do
    local aStage = tostring(a.stage or ""):upper()
    if aStage == STU then
      local head = tonumber(a.head or a.count or a.n or 0) or 0
      if head > 0 then
        local gRaw = tostring(a.gender or a.sex or ""):lower()
        local g = (gRaw == "female" or gRaw == "male") and gRaw or nil
        local flags = a.flags or {}
        local isCast = (flags.castrated == true or flags.isCast == true or flags.isCastrated == true
                       or a.castrated == true   or a.isCast == true     or a.isCastrated == true)
        local age = tonumber(a.ageM or a.ageMonths or a.age_mo or a.age or 0) or 0

        if g == "female" then
          fAll = fAll + head
          if age >= snrM then fOver = fOver + head end
        elseif g == "male" and isCast then
          mAll = mAll + head
          if age >= overM then mOver = mOver + head end
        end
      end
    end
  end

  if fAll == 0 and mAll == 0 then return "" end

  local fracF = (fAll > 0) and (fOver / fAll) or 0
  local fracM = (mAll > 0) and (mOver / mAll) or 0
  local factor = ((1 - fracM) + fracM * math.min(1, floor))
  factor       = factor * ((1 - fracF) + fracF * math.min(1, floor))

  if factor >= 0.995 then return "" end
  local pct = math.floor((1 - factor) * 100 + 0.5)
  return string.format(" | price -%d%%", pct)
end

PN_UI.stageOrder = PN_UI.stageOrder or {
  COW     = { CALF=1, HEIFER=2, COW_LACT=3, COW_DRY=4, YEARLING=5, STEER_FINISH=6, BULL=7, STEER_OVERAGE=8, COW_OVERAGE=9, OVERAGE=10, DEFAULT=99 },
  SHEEP   = { LAMB=1, WEANER=2, EWE_LACT=3, EWE_DRY=4, RAM=5, WETHER_FINISH=6, RAM_ADULT=7, WETHER_OVERAGE=8, EWE_OVERAGE=9, OVERAGE=10, DEFAULT=99 },
  PIG     = { PIGLET=1, GILT=2, SOW_LACT=3, SOW_DRY=4, BARROW_FINISH=5, BOAR=6, BOAR_ADULT=7, BARROW_OVERAGE=8, SOW_OVERAGE=9, OVERAGE=10, DEFAULT=99 },
  GOAT    = { KID=1, DOE_LACT=2, DOE_DRY=3, GROWER=4, WETHER_FINISH=5, BUCK=6, BUCK_ADULT=7, WETHER_OVERAGE=8, DOE_OVERAGE=9, OVERAGE=10, DEFAULT=99 },
  HORSE   = { FOAL=1, WEANLING=2, YEARLING=3, MARE_LACT=4, MARE_DRY=5, COLT=6, GELDING=7, STALLION=8, GELDING_OVERAGE=9, MARE_OVERAGE=10, OVERAGE=11, DEFAULT=99 },
...
-- NEW: Effects summary row (PN_Effects first, PN_Core fallback)
function PN_UI._effectsSummaryRow(e)
  if not e then return nil end

  -- Prefer PN_Effects summary if available
  if PN_Effects and type(PN_Effects.getBarnSummary) == "function" then
    local ok, s = pcall(PN_Effects.getBarnSummary, PN_Effects, e)
    if ok and type(s) == "table" then
      local gm     = tonumber(s.growthMult   or 0)
      local fertPP = tonumber(s.fertDeltaPP  or 0)
      local hpd    = tonumber(s.healthPerDay or 0)
      local fe     = tonumber(s.feDMperKg    or 0)
      local stress = tostring(s.stressLabel  or "—")
      local text = string.format(
        "Effects: Growth×%0.2f | Fert %+dpp | Health %+0.2f/d | FE %0.1f kgDM/kg | Stress %s",
        gm, math.floor((fertPP or 0) + 0.5), hpd, fe, stress
      )
      local bad = (gm > 0 and gm < 1.0) or (hpd < 0)
      return { gender = bad and "warn" or "feed", text = text }
    end
  end

  -- Fallback: pull robust totals and accept many aliases (incl. nested)
  local t        = PN_UI._getBarnTotals(e)
  local head     = _numPick(t, {"animals","head","n","counts.animals"}) or 0

  -- Nutrition ratio (0..1). If missing in totals, fall back to the snapshot field.
  local nut = _numPick(t, {"nut","nutRatio","nutrition","ratios.nut"})
  if (nut == nil or nut <= 0) and e.__pn_last and e.__pn_last.nutRatio then
    nut = tonumber(e.__pn_last.nutRatio) or nut
  end
  nut = nut or 0

  -- Average daily gain (kg/d)
  local adg = _numPick(t, {"adg","ADG","growth.adg","perHead.ADG"}) or 0

  -- DMI per head (kg/head/day)
  local dmiHd = _numPick(t, {
    "intakeKgHd","DMIhd","dmiHd","dmi_hd","dmiPerHead","dmi_head","perHead.DMI","perHead.dmi"
  })
  if (not dmiHd) or dmiHd <= 0 then
    local dmiTot = _numPick(t, {"DMIt","DMI","dmiTotal","dmi_tot","totals.DMI","totals.dmi"})
    if dmiTot and dmiTot > 0 and head > 0 then dmiHd = dmiTot / head end
  end

  -- Energy & Protein (accept totals or per-head; show totals first if both exist)
  -- Canonical first
  local E = _numPick(t, {"energyMJ","totals.energyMJ"}) or _pickEnergyMJ(t)
  local P = _numPick(t, {"proteinKg","totals.proteinKg"}) or _pickProteinKg(t)


  -- Build label; show "—" if truly unavailable
  local parts = {
    string.format("Effects: Nut=%d%%", math.floor((nut or 0)*100 + 0.5)),
    string.format("ADG=%s", (adg and adg ~= 0) and string.format("%0.2f kg/d", adg) or "—"),
    string.format("DMI/head=%s", (dmiHd and dmiHd > 0) and string.format("%0.2f kg", dmiHd) or "—"),
    string.format("E=%s", (E and E > 0) and (tostring(math.floor(E + 0.5)).." MJ") or "—"),
    string.format("P=%s", (P and P > 0) and string.format("%0.2f kg", P) or "—"),
  }

  local text = table.concat(parts, " | ")
  local warn = (nut or 0) < 0.80
  return { gender = warn and "warn" or "feed", text = text }
end
...
-- Helper: show "(ovr)" if PN_Core applied an overage penalty for the relevant sex
local function _normalizeStage(species, gender, stage, flags)
  local SU = tostring(species or "ANIMAL"):upper()
  local U  = tostring(stage or ""):upper()
  if SU == "COW" and U == "OVERAGE" then
    -- Legacy Core/UI could emit generic OVERAGE; make it explicit by sex.
    local g = (gender == "male" or gender == "female") and gender or nil
    local isCast = flags and (flags.castrated == true)
    if g == "male" or isCast then
      return "STEER_OVERAGE"
    else
      return "COW_OVERAGE"
    end
...
local function makeStageRows(e)
  local pregByStage = (e and e.__pn_last and e.__pn_last.preg and e.__pn_last.preg.byStage) or nil
  local rows = {}
  ...
  if e.__pn_last and type(e.__pn_last.animals) == "table" and #e.__pn_last.animals > 0 then
    for _, a in ipairs(e.__pn_last.animals) do
      if type(a) == "table" then
        ...
        stage = _normalizeStage(species, gender, stage, flags)
        a.stage = stage  -- carry normalized stage for suffix/preg lookups
  ...
  -- Emit rows for this barn:
  local rows = {}

  -- 1) Per-stage rows (with palette tags)
  ...
  for _, st in ipairs(keys) do
    ...
    -- Tagging and row text (species-aware)
    local tag
    local SUu = tostring(species or "ANIMAL"):upper()
    local U   = tostring(st):upper()

    if SUu == "SHEEP" and U == "YEARLING" and (t.male > 0 or t.female > 0) then
      if t.female > 0 then
        local nf   = t.female
        local avgF = (t.wSumF > 0) and (t.wSumF / math.max(1, nf)) or 0
        local __rowText = string.format("%s [%s/%s] | head=%d%s | avgW=%.2fkg | Nut=%d%% | ADG=%.3f kg/d",
          name, species, st, nf,
          (function() local k=tostring(st):upper(); local n=(pregByStage and pregByStage[k]) or 0; return (n>0 and string.format(" [preg:%d]", n) or "") end)(),
          avgF, nutPct, adg)
        __rowText = __rowText .. _pn_priceSuffixForStage(e, species, st)
        table.insert(rows, { gender = "femaleOpen", text = __rowText })
      end
      if t.male > 0 then
        local nm   = t.male
        ...
        local __rowText = string.format("%s [%s/%s] | head=%d%s | avgW=%.2fkg | Nut=%d%% | ADG=%.3f kg/d",
          ...
          avgM, nutPct, adg)
        __rowText = __rowText .. _pn_priceSuffixForStage(e, species, st)
        table.insert(rows, { gender = "male", text = __rowText })
      end

    elseif SUu == "PIG" and U == "FINISHER" and (t.male > 0 or t.female > 0) then
      if t.female > 0 then
        local nf   = t.female
        local avgF = (t.wSumF > 0) and (t.wSumF / math.max(1, nf)) or 0
        local __rowText = string.format("%s [%s/%s] | head=%d%s | avgW=%.2fkg | Nut=%d%% | ADG=%.3f kg/d",
      ...
      elseif SUu == "GOAT" then
        if U == "KID" or U == "WEANER" then
          tag = "calf"
        elseif U == "DOE_LACT" then
          tag = "femalepreg"
        elseif U == "DOE_DRY" then
          tag = "femaleOpen"
        elseif U == "WETHER" or U == "WETHER_FINISH" or U == "BUCK_GROW" or U == "BUCK_ADULT" then
          tag = "male"
        else
          if U:find("DOE",1,true) then
            tag = "femaleOpen"
          elseif U:find("KID",1,true) or U:find("WEAN",1,true) then
            tag = "calf"
          else
            tag = "male"
          end
      ...
      elseif SUu == "HORSE" then
        if U == "FOAL" or U == "WEANLING" or U == "YEARLING" then
          tag = "calf"
        elseif U == "MARE_LACT" or U == "LACT" then
          tag = "femalepreg"
        elseif U == "MARE_DRY" or U == "DRY" then
          tag = "femaleOpen"
        elseif U == "GELDING" or U == "COLT" or U == "STALLION" then
          tag = "male"
        elseif U == "GELDING_OVERAGE" then
          tag = "male"
        else
          if U:find("MARE", 1, true) then
            tag = "femaleOpen"
          elseif U:find("FOAL",1,true) or U:find("WEAN",1,true) or U:find("YEAR",1,true) then
            tag = "calf"
          else
            tag = "male"
          end
        ...
        elseif U == "LACT" or U == "GEST" then
          tag = "femalepreg"
        elseif U == "COW_OVERAGE" then
          tag = "femaleOpen"
        elseif U == "STEER_OVERAGE" or U == "OVERAGE" then
          tag = "male"
        else
          tag = "male"
        end
      ...
      local __rowText = string.format("%s [%s/%s] | head=%d%s | avgW=%.2fkg | Nut=%d%% | ADG=%.3f kg/d",
        name, species, tostring(st), tonumber(t.head or 0),
        (function() local k=tostring(st):upper(); local n=(pregByStage and pregByStage[k]) or 0; return (n>0 and string.format(" [preg:%d]", n) or "") end)(),
        tonumber(avgW or 0), nutPct, adg)
      __rowText = __rowText .. _pn_priceSuffixForStage(e, species, st)
      table.insert(rows, { gender = tag, text = __rowText })
    end
  end
...
local function buildFallbackRows()
  ...
  for _, e in ipairs(toDraw) do
    ...
    -- Effects line (first), optional in list mode
    if PN_UI.cfg.showEffectsRow and (PN_UI.mode == "single" or not PN_UI.cfg.effectsOnlySingle) then
      local fx = PN_UI._effectsSummaryRow(e)
      if fx and fx.text then table.insert(rows, fx) end
    end
...
-- Helper: respect basegame HUD visibility (so HideHud hides PN too)
local function _pn_isBaseHudVisible()
  local hud = g_currentMission and g_currentMission.hud
  if hud ~= nil then
    if type(hud.getIsVisible) == "function" then
      local ok, vis = pcall(function() return hud:getIsVisible() end)
      if ok then return vis end
    end
    if hud.isVisible ~= nil then return hud.isVisible end
  end
  -- fallback: assume visible if unsure
  return true
end
function PN_UI.onDraw()
  if not PN_UI.enabled then return end
  if not _pn_isBaseHudVisible() then return end
  ...
  if PN_UI.mode == "single" then
    ...
    -- include the help hint explicitly
    header = string.format("[PN] %s (%d/%d) – %s  [%s] %s",
      title, idx, #list, mode, ac, "(ALT+B/M to cycle barns)")
  else
    -- list mode: no per-barn counters; keep the classic title
...
if not PN_UI.__overlayRegistered then
  function PN_UI:cmdOverlay(args, ...)
    ...
    elseif sub == "effects" then
      local v = (a[2] or ""):lower()
      if     v == "on"  then PN_UI.cfg.showEffectsRow = true;  Logging.info("[PN:Overlay] effects: on")
      elseif v == "off" then PN_UI.cfg.showEffectsRow = false; Logging.info("[PN:Overlay] effects: off")
      else
        Logging.info("[PN:Overlay] effects on|off (currently %s)", PN_UI.cfg.showEffectsRow and "on" or "off")
      end
    ...
    elseif sub == "save" then
      local name = a[2] or "config"
      if PN_Config and PN_Config.saveUI then
        local ok, err, p = PN_Config:saveUI(name)
        if ok then
          Logging.info("[PN:Overlay] Saved UI config → %s", tostring(p))
        else
          Logging.info("[PN:Overlay] Save failed (%s)", tostring(err or "?"))
        end
      else
        Logging.info("[PN:Overlay] Save not available (PN_Config missing).")
      end
    elseif sub == "load" then
      local name = a[2] or "config"
      if PN_Config and PN_Config.loadUI then
        local ok, err, p = PN_Config:loadUI(name)
        if ok then
          Logging.info("[PN:Overlay] Loaded UI config ← %s", tostring(p))
        else
          Logging.info("[PN:Overlay] Load failed (%s).", tostring(err or "not-found"))
        end
      else
        Logging.info("[PN:Overlay] Load not available (PN_Config missing).")
      end
    elseif sub == "list" or sub == "ls" then
      if PN_Config and PN_Config.list then
        local names = PN_Config:list()
        if #names == 0 then
            Logging.info("[PN:Overlay] No configs found in modSettings/FS25_PrecisionNutrition")
        else
            Logging.info("[PN:Overlay] Available UI configs:")
            for _,n in ipairs(names) do Logging.info("  - %s", tostring(n)) end
        end
      else
        Logging.info("[PN:Overlay] List not available (PN_Config missing).")
      end
    ...
    elseif sub == "probe" then
      ...
      Logging.info("[PN:Overlay] Usage: pnOverlay on|off|toggle | effects on|off | alpha <0..1> | anchor tl|tr|bl|br | margin <x y> | rows <N> | fontsize <v> | rowH <v> | padding <x y> | width auto|<v>|min <v>|max <v>|clamp <min> <max> | reset | status")
    end
