...
PN = PN or {}         -- ensure global exists before accessing PN.tuning
PN_Core = PN_Core or {}
...
PN_Core._warnedFT    = PN_Core._warnedFT or {}      -- tracks unknown feed tokens we've warned about
PN_Core._lastHourPNGF = PN_Core._lastHourPNGF or -1       -- hour edge tracker for grazing feeder
...
PN_Core.SCHEMA = {
  totals = {
    animals     = "animals",      -- head
    nut         = "nut",          -- 0..1
    adg         = "adg",          -- kg/day (avg per head, or barn-level avg)
    DMIt        = "DMIt",         -- kg/day total barn (target; legacy)
    intakeKgHd  = "intakeKgHd",   -- kg/day per head (actual)
    energyMJ    = "energyMJ",     -- MJ/day total barn (actual)
    proteinKg   = "proteinKg",    -- kg/day total barn (actual)
  },
  perHead = {
    energyMJ    = "perHead.energyMJ", -- MJ/day per head (actual)
    proteinKg   = "perHead.proteinKg" -- kg/day per head (actual)
  },
  units = {
    nut="ratio(0..1)", adg="kg/d", DMIt="kg/d", intakeKgHd="kg/hd/d",
    energyMJ="MJ/d", proteinKg="kg/d", perHead_energyMJ="MJ/hd/d", perHead_proteinKg="kg/hd/d"
  }
}

-- === Nutrition → Effects helpers (Alpha) ===
local function PN_nutToAdgMultiplier(nut)
    -- piecewise linear: [0..0.5] maps to adgMin..adgAtHalf, (0.5..1] maps to adgAtHalf..(1+maxBoost)
    local t = (PN and PN.tuning) or {}
    nut = math.max(0, math.min(1, nut or 0))
    local a0  = t.adgMinFactorAtZero or 0.20
    local a50 = t.adgFactorAtHalf    or 0.80
    local a1  = 1 + (t.adgMaxBoost   or 0.20)

    if nut <= 0.5 then
        -- from Nut 0.0 → 0.5 map to a0 → a50
        return a0 + (a50 - a0) * (nut / 0.5)
    else
        -- from Nut 0.5 → 1.0 map to a50 → a1
        return a50 + (a1 - a50) * ((nut - 0.5) / 0.5)
    end
end

local function PN_applyConditionDrift(barnKey, nut, dtHours)
    PN._barnMem = PN._barnMem or {}
    local mem = PN._barnMem[barnKey] or { condition = 1.0 }
    local t = (PN and PN.tuning) or {}
    local c = mem.condition or 1.0

    if nut < 0.6 then
        c = c - (t.conditionDriftPerHr or 0.001) * dtHours
    elseif nut > 0.9 then
        c = c + (t.conditionDriftPerHr or 0.001) * dtHours
    end

    local cMin = t.conditionMin or 0.50
    local cMax = t.conditionMax or 1.00
    mem.condition = math.max(cMin, math.min(cMax, c))
    PN._barnMem[barnKey] = mem
    return mem.condition
end

-- -----------------------
-- Utils
-- -----------------------
-- Hour-edge hook to run the grazing feeder once per in-game hour
function PN_Core.hourEdgeGraze()
    if not PN_GrazingFeeder or not PN_GrazingFeeder.onHourChanged then return end
    local env = g_currentMission and g_currentMission.environment
    if not env or env.currentHour == nil then return end
    local h = tonumber(env.currentHour or -1) or -1
    if h ~= PN_Core._lastHourPNGF then
        PN_Core._lastHourPNGF = h
        pcall(PN_GrazingFeeder.onHourChanged)
    end
end
...
-- Helper: treat RL flags like "true"/1 as truthy
local function _truthy(v)
  return v == true or v == 1 or v == "1" or v == "true" or v == "TRUE"
end

-- Compute pregnancy summary from the latest per-animal snapshot
local function _pn_pregSummary(entry)
  local total, byStage = 0, {}
  ...
  if type(animals) == "table" then
    for _, a in ipairs(animals) do
      ...
      if head > 0 and _truthy(flags.isPregnant) then
        total = total + head
        local st = tostring(a.stage or "DEFAULT"):upper()
        byStage[st] = (byStage[st] or 0) + head
      end
    end
  end
...
-- RL parity reader: true if the animal has calved at least once
local function _pn_isParent(x)
    if not x then return false end
    local m = x.memory
    local function truth(v) return v == true or (type(v)=="number" and v > 0) end

    -- direct cluster flags
    if x.isParent ~= nil          then return truth(x.isParent) end
    if x.hasCalved ~= nil         then return truth(x.hasCalved) end
    if x.parity ~= nil            then return truth(x.parity) end
    if x.lactations ~= nil        then return truth(x.lactations) end
    if x.lactationNumber ~= nil   then return truth(x.lactationNumber) end

    -- memory mirror
    if m then
        if m.isParent ~= nil        then return truth(m.isParent) end
        if m.hasCalved ~= nil       then return truth(m.hasCalved) end
        if m.parity ~= nil          then return truth(m.parity) end
        if m.lactations ~= nil      then return truth(m.lactations) end
        if m.lactationNumber ~= nil then return truth(m.lactationNumber) end
    end
    return false
end
...
-- Pull the active COW knobs from PN_Settings.meta
local function _pn_getCowKnobs()
  local M = (PN_Settings and PN_Settings.meta and PN_Settings.meta.COW) or {}
  local sale = M.sale or {}
  return {
    steerCut   = tonumber(M.overageM)            or 26,
    cowSnr     = tonumber(M.overageSnr)          or 60,
    perMonth   = tonumber(sale.overagePenaltyPerMonth) or 0.03,
    floor      = tonumber(sale.overagePenaltyFloor)    or 0.80,
  }
end

-- Estimate how much of a bucket (entry) is "steer" (male+castrated) at/over a given age.
-- Returns a fraction in [0..1] based on cluster counts.
local function _pn_castratedFracFor(entry, minMonths)
  local cs = entry and entry.clusterSystem
  if not (cs and cs.getClusters) then return 0 end
  local ok, clusters = pcall(cs.getClusters, cs)
  if not ok or type(clusters) ~= "table" then return 0 end

  local steer, total = 0, 0
  for _, c in pairs(clusters) do
    if type(c)=="table" and not c.isDead then
      local g  = c.gender
      if type(g)=="function" then local ok2,v=pcall(g,c); if ok2 then g=v end end
      local male = (g==1) or (g==true) or (tostring(g or ""):lower()=="male" or tostring(g or ""):lower()=="m")
      if male then
        local months = tonumber(c.ageMonths or c.months or 0) or 0
        if months >= (minMonths or 0) then
          total = total + 1
          local cas = c.isCastrated or c.castrated or c.isCast
          if type(cas)=="function" then local ok3,v=pcall(cas,c); if ok3 then cas=v end end
          cas = (cas==true) or (cas==1) or (tostring(cas or ""):lower()=="true")
          if cas then steer = steer + 1 end
        end
      end
    end
  end
...
function PN_Core.resolveStage(self, species, gender, ageM, flags)
    ...
    if species == "COW" then
		...
		elseif gender == "female" then
			-- Over-age guard stays first if you use it
			if (ageM or 0) >= 60 then return "COW_OVERAGE" end

			local isLact   = flags and (flags.lactating == true)
			local isParent = flags and (flags.parent == true)

			-- 1) Lactation always wins
			if isLact then
				return "LACT"
			end

			-- 2) If RL parity says she's calved before, it's a cow → DRY
			if isParent == true then
				return "DRY"
			end

			-- 3) No parity info (false/nil): fall back to age bands for visibility
			--    (so adult never-calved or unknown-parity animals don't all look like HEIFER)
			if (ageM or 0) >= 18 then
				return "DRY"
			end

			-- 4) Default: replacement heifer
			if (ageM or 0) >= 6 then
				return "HEIFER"
			end
			-- <6m falls through to CALF via unsexed band
    ...
    if g.head > 0 then
        ...
        if mk and mk > 0 and g.avgW > 0 then
            ...
            a = a * reserve
		        ...
		        -- ---------- Overage ADG penalty (steers @ overageM; cows @ overageSnr) ----------
        do
          local U = (tostring(speciesKey or "COW")):upper()
          if U == "COW" then
            local K      = _pn_getCowKnobs()
            local months = math.max(0, math.floor(tonumber(gMonths or 0) or 0))

            -- Penalty function (bounded by floor)
            local function penaltyFactor(monthsOver)
              if monthsOver <= 0 then return 1.0 end
              return math.max(1.0 - monthsOver * K.perMonth, K.floor)
            end

            if genderKey == "male" then
              -- Steers only (male + castrated) get the 26m+ penalty.
              if months >= K.steerCut then
                local fracCastrated = _pn_castratedFracFor(entry, K.steerCut)
                if fracCastrated > 0 then
                  local f = penaltyFactor(months - K.steerCut)
                  -- Mix: only the castrated portion of the bucket is penalized.
                  a = a * ( (1.0 - fracCastrated) + fracCastrated * f )
                end
              end
            else
              -- Females (and intact males if you want them included): senior penalty from 60m+
              if months >= K.cowSnr then
                local f = penaltyFactor(months - K.cowSnr)
                a = a * f
              end
            end
          end
		...
		-- Optional debug flag/note to show overage penalty was applied
		if (speciesKey or "COW") == "COW" then
			g._dbgOverageNote = " (ovr)"
		end
    end
    return g
end
...
function PN_Core.updateHusbandry(self, entry, clusterSystem, dtMs, ctx)
	...
	-- Track every entry we touch so debug commands can see them even if the game’s list is empty
	PN_Core._entries = PN_Core._entries or {}
	if entry then
		-- Use the object itself as the key so we preserve identity, but also store a readable name
		if PN_Core._entries[entry] == nil then
			PN_Core._entries[entry] = {
				ref  = entry,
				name = tostring(entry.name or entry.id or entry.placeableId or "Husbandry"),
			}
		end
		-- Keep the display name fresh if it changes
		local e = PN_Core._entries[entry]
		e.name = tostring(entry.name or e.name)
	end
    ...
    local E_tgtF2, P_tgtF2, DMI_tgtF = _getTargets(species, stageF.name)
    local E_tgtM2, P_tgtM2, DMI_tgtM = _getTargets(species, stageM.name)
    ...
    local reqDmBarn = reqDmF + reqDmM
        ...
        -- Barn-level targets (per-day totals; do not scale by frame)
    local E_tgtBarnDay = ((E_tgtF2 or 0) * math.max(0, cows)  + (E_tgtM2 or 0) * math.max(0, bulls))
    local P_tgtBarnDay = ((P_tgtF2 or 0) * math.max(0, cows)  + (P_tgtM2 or 0) * math.max(0, bulls))
    local D_tgtBarnDay = math.max(0, (DMI_tgtF or 0) * math.max(0, cows) + (DMI_tgtM or 0) * math.max(0, bulls))

    -- 5) Fulfillment (and canonical per-day actuals)
    local eRat, pRat, dRat, nut
    local E_day, P_day, D_day -- canonical actuals scaled to per-day
    local scale = (dtH > 0) and (24 / dtH) or 0

    if (N.dm or 0) > 0 then
        -- Use measured consumption (delta path) → scale up to per-day
        E_day = (N.energyMJ or 0)  * scale
        P_day = (N.proteinKg or 0) * scale
        D_day = (N.dm or 0)        * scale

        eRat = (E_tgtBarnDay > 0) and (E_day / E_tgtBarnDay) or 0
        pRat = (P_tgtBarnDay > 0) and (P_day / P_tgtBarnDay) or 0
        dRat = (D_tgtBarnDay > 0) and (D_day / D_tgtBarnDay) or 0
        nut  = math.max(0, math.min(1, math.min(eRat, math.min(pRat, dRat))))
    else
        -- Estimate from trough composition assuming draw to demand if available
        if reqDmBarn > 0 and availDm > 0 then
            local totalDm = 0
            local dmByFt = {}
            for ftIndex, liters in pairs(levels or {}) do
                local L = tonumber(liters or 0) or 0
                if L > 0 then
                    local asFedKg = _kgFromFill(ftIndex, L)
                    local _, row = _resolveFeed(ftIndex)
                    if row and row.dm and row.dm > 0 and asFedKg > 0 then
                        local dmKg = asFedKg * (row.dm or 1)
                        if dmKg > 0 then
                            dmByFt[ftIndex] = dmKg
                            totalDm = totalDm + dmKg
            ...
            local drawDm = math.min(reqDmBarn, availDm) -- frame amount (DM kg)
            local E_est, P_est = 0, 0
            if totalDm > 0 and drawDm > 0 then
                for ftIndex, dmKg in pairs(dmByFt) do
                    local share   = dmKg / totalDm
                    local dmAlloc = drawDm * share
                    local _, row  = _resolveFeed(ftIndex)
                    if row and row.dm and row.dm > 0 then
                        local asFedAlloc = dmAlloc / row.dm
                        E_est = E_est + (asFedAlloc * (row.energyMJ  or 0))
                        P_est = P_est + (asFedAlloc * (row.proteinKg or 0))
                    end
                end
            end

            -- Scale estimates up to per-day canonicals
            E_day = (E_est or 0) * scale
            P_day = (P_est or 0) * scale
            D_day = (drawDm or 0) * scale
            ...
            eRat, pRat, dRat, nut = 0, 0, 0, 0
            E_day, P_day, D_day   = 0, 0, 0
        end
    end
	...
	-- === PN Alpha Effects: derive ADG multiplier & effects from Nut ===
    -- Create a stable barn key; prefer explicit ids if present
    local barnKey = entry and (entry.id or entry.clusterId or entry.placeableId) or tostring(entry)

    -- Per-frame hours already computed as dtH
    local adgMulBase = PN_nutToAdgMultiplier(nut)
    local condition  = PN_applyConditionDrift(barnKey, nut, dtH)
    local adgMul     = adgMulBase * condition
    adgMul = math.max(0.10, math.min(1.25, adgMul))  -- safety clamp for alpha

    -- Simple 0..100 effects score for UI/price signalling
    local effects = math.floor((nut or 0) * 100 + 0.5)

    -- Publish for overlay / downstream use
    entry.__pn_effects  = effects
    entry.__pn_adgMul   = adgMul
    entry.__pn_condition= condition

    -- Optional: expose milk scaling if your output pipeline consumes it (no side effects otherwise)
    local t = PN.tuning or {}
    entry.__pn_milkMul = math.min(
        (t.milkBoostClamp or 1.20),
        0.70 + (t.milkBoostMax or 0.30) * (nut or 0)
    )


    -- 7) Compute ADG using nutrition-derived multiplier, then taper by mature weight
    local baseADG = tonumber(stage.baseADG or 0) or 0
    local adg = baseADG * (entry.__pn_adgMul or (nut or 0))
    ...
    -- ---- Canonical actuals (per-head/day & totals/day) ----------------------
    local intakeKgHd = (head > 0) and (D_day / head) or 0
    local perHeadE_MJ = (head > 0) and (E_day / head) or 0
    local perHeadP_kg = (head > 0) and (P_day / head) or 0

    -- 10) Persist totals for UI/debug (keep legacy fields; add canonical keys)
    entry.__pn_totals = {
        animals   = head,
        bulls     = bulls,
        cows      = cows,
        ...
        avgWeight = avgW,
        nut       = nut,
        adg       = adg,          -- kg/day (avg per head)
        -- legacy instantaneous bins (pre-scale); left for backward compatibility
        E = (N.energyMJ or 0), P = (N.proteinKg or 0), DMI = (N.dm or 0),
        Et = E_tgt, Pt = P_tgt, DMIt = DMI_tgt,
        -- canonical per-day actuals
        intakeKgHd = intakeKgHd,     -- kg/hd/day
        energyMJ   = E_day,          -- MJ/day barn
        proteinKg  = P_day,          -- kg/day barn
        perHead    = {
          energyMJ  = perHeadE_MJ,   -- MJ/hd/day
          proteinKg = perHeadP_kg    -- kg/hd/day
        },
        species   = species,
        stage     = stage.name,
        gender    = barnGender,
    ...
    entry.__pn_last.effects  = entry.__pn_effects          -- NEW
    entry.__pn_last.adgMul   = entry.__pn_adgMul           -- NEW
            ...
            for _, c in pairs(clusters) do
                if type(c) == "table" and not c.isDead then
                    ...
                    local flags = {
					    ...
					    parent    = _pn_isParent(c),  -- ← has calved at least once (parity)	
                    
                        isPregnant = _truthy(c.isPregnant) or _truthy(c.pregnant),
                    }
    ...
    entry.__pn_last.preg = _pn_pregSummary(entry)
    end

	-- Build per-stage aggregates (head, avgW, and the exact ADG we use in core)
	do
		local animals = entry.__pn_last and entry.__pn_last.animals or {}
		local byStage = {}
		for _, a in ipairs(animals) do
			...
			if not dst then
				dst = { stage = st, gender = g, n = 0, wSum = 0, adg = 0 }
				byStage[key] = dst
			end
			dst.n    = dst.n + head
		...
		-- use the same ADG logic your core uses (like pnBeat), per stage
		local speciesResolved2 = (entry.__pn_totals and entry.__pn_totals.species) or species
		for _, dst in pairs(byStage) do
			local n = dst.n
			if n > 0 then
				local avgWst = (dst.wSum > 0 and (dst.wSum / n)) or 0

				-- reproduce groupADG logic: stage baseADG * stage nut * maturity taper
				local stObj = _getStageForEntry(speciesResolved2, avgMonths, dst.gender, entry)
				local base  = tonumber(stObj.baseADG or 0) or 0

				-- per-stage nut: use stage-aware path if provided, else fall back to barn nut
				local nutStage
				if PN_Core and PN_Core.nutritionForStage then
					local okN, r = pcall(PN_Core.nutritionForStage, PN_Core, speciesResolved2, dst.stage, entry)
					if okN and type(r)=="number" then nutStage = math.max(0, math.min(1, r)) end
				end
				nutStage = nutStage or (entry.__pn_last and entry.__pn_last.nutRatio) or 0

				local adgSt = base * nutStage

				local meta = PN_Core.meta and PN_Core.meta[(speciesResolved2 or "COW"):upper()] or nil
				local mk = meta and tonumber(meta.matureKg) or nil
				if mk and mk > 0 and avgWst > 0 then
					local frac = math.max(0, math.min(1, avgWst / mk))
					local reserve = math.max(0.10, 1.0 - (frac ^ 1.6))
					adgSt = adgSt * reserve
				end
